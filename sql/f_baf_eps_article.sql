CREATE OR REPLACE PROCEDURE F_BAF_EPS_ARTICLE (
...
AS
	...
	ls_typ_ord GEO_ORDRE.TYP_ORDRE%TYPE;
...
BEGIN
	...
	select TYP_ORDRE into ls_typ_ord
	from GEO_ORDRE
	where ORD_REF =arg_ord_ref;

	-- le 30/11/22 BRUNO AMADEI
	-- on ne contrôle pas ni de maj quand c est un ordre de regul
	If ls_typ_ord = 'RPO' or  ls_typ_ord = 'RPR' or  ls_typ_ord = 'REG' then
		res := 1;
		return;
	end if;
