...
--		dw_table.object.acces1_1.visible = FALSE
	...
	--if is_flu_code = 'ORDRE' then
	if is_flu_code = 'ORDRE' and  ib_ann_ordre = FALSE	then
	...
	if ib_ann_ordre = FALSE then --LLEF annulation ordre
...
CREATE OR REPLACE PROCEDURE GEO_PREPARE_ENVOIS (
	...
	is_flu_code IN GEO_FLUX.FLU_CODE%TYPE, 
	mode_auto IN boolean,
	ann_ordre IN boolean,
	arg_nom_utilisateur GEO_USER.NOM_UTILISATEUR%TYPE,
	res OUT number,
	msg OUT varchar2
)
AS 
	ls_typ_ordre GEO_TYPORD.TYP_ORD%TYPE;
	ib_ann_ordre boolean;
BEGIN
