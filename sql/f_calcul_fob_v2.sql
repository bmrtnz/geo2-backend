CREATE OR REPLACE PROCEDURE "GEO_ADMIN".F_CALCUL_FOB_V2(
...
AS
    ...
    li_nb_fob number;
    ls_flannul GEO_ORDRE.FLANNUL%TYPE;
...
BEGIN
    ...
    select FLANNUL into ls_flannul
    from GEO_ORDRE
    WHERE ORD_REF = is_ord_ref;

    if (ls_flannul = 'O') then
        msg := 'Impossible sur un ordre annulé';
        return;
    end if;
