...
begin
    ...
    select ol.propr_code, ol.ord_ref, ol.art_ref, o.tvt_code, o.sco_code, ol.vte_bta_code, ol.ach_bta_code, o.typ_ordre, ol.fou_code, o.cen_ref, ol.orl_lig, o.trp_code
    into ls_val, ls_ord_ref, ls_art_ref, ls_tvt_code, ls_sco_code, ls_vte_bta, ls_ach_bta, ls_typ_ordre, ls_fou_code_old, ls_cen_ref, ls_orl_lig, current_transporteur
    from geo_ordlig ol
    ...
    begin
        ...
        if ld_prix_mini > 0 and ld_prix_mini is not null and arg_soc_code <> 'IMP' and arg_soc_code <> 'BUK' and ls_sco_code <> 'RET' then
            ...
            select coalesce(ach_dev_pu, 0) into ld_prix_mini from geo_ordlig where orl_ref = arg_orl_ref;

            update geo_ordlig
            set ach_pu = ld_prix_mini * ld_dev_taux
            where orl_ref = arg_orl_ref;
            commit;
        end if;
        --fin marketing
    ...
    /*
    if ls_orl_lig = '01' and (current_transporteur is null or current_transporteur = '-') then
