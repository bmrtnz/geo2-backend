...
BEGIN
    ...
    begin
        ...
        loop
            ...
            ls_sql := ls_sql || ' select K_STOCK_ART_EDI_BASSIN, ALERTE_PAL, ALERT_PAL_MESS, FLAG_HORS_BASSIN, GTIN, ART_REF ';
            ...
            declare
                k_seb number;
                alert_pal GEO_STOCK_ART_EDI_BASSIN.ALERTE_PAL%TYPE;
                alert_msg GEO_STOCK_ART_EDI_BASSIN.ALERT_PAL_MESS%TYPE;
            begin
                OPEN C_LIG_EDI_L FOR to_char(ls_sql);
                loop
                    fetch C_LIG_EDI_L into k_seb, alert_pal, alert_msg;
                    EXIT WHEN C_LIG_EDI_L%notfound;
                    if alert_pal = 'O' then
                        alerts_pal_msg := alerts_pal_msg || ', ' || alert_msg;
                    end if;
    ...
    if alerts_pal_msg is not null then
        res := 2;
        msg := alerts_pal_msg;
    end if;
