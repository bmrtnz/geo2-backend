        ...
        LOOP
			...
			if ll_ident_fou = 4 or ll_ident_fou = 5 then -- "Plateforme import", "Plateforme France"
				...
				if ls_station_pal_code <> ls_entrep_pal_code and ls_entrep_pal_code is not null and  length(ls_entrep_pal_code) > 0 then
					...
					ls_pal_code_sauve := ls_entrep_pal_code;
				end if;
			else
				if ls_station_pal_code = ls_entrep_pal_code  then 
					ls_a_traiter := 'O';
					ls_pal_code_sauve := ls_entrep_pal_code;
				else
					if ls_entrep_pal_code is null or length(ls_entrep_pal_code) = 0 then
						ls_a_traiter := 'O';
						ls_pal_code_sauve := '';
					else
						ls_a_traiter := 'N';
			...
			if ls_a_traiter = 'O' then
				...
				if ls_retour_ctrl_insert_bws = 'OK' then
                    declare
                        save_res number;
                    begin
                        f_sauve_stock(ls_sto_ref, arg_ref_edi_ordre, arg_ref_edi_ligne, arg_art_ref, arg_cam_code, arg_type_recherche, arg_bws_ecris, ls_pal_code_sauve, ls_alerte_pal, ls_alerte_pal_mess, save_res, msg);  -- Return 'OK' si insert effectué
                        if save_res = 0 then
                            res:= 0;
                            return;
                        else
