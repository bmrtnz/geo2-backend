CREATE OR REPLACE PROCEDURE GEO_ADMIN.OF_GENERE_ENVOIS (
    ...
    env_codes IN OUT p_str_tab_type
)
AS
    is_base_flu_code GEO_FLUX.FLU_CODE%TYPE;
...
BEGIN
    is_base_flu_code := is_flu_code;
    env_codes := p_str_tab_type();

    if (is_flu_code = 'FICPAO' or is_flu_code = 'FICPAN') then
        is_flu_code := 'FICPAL';
    end if;
    ...
    begin
        ...
        select A.soc_code, A.cam_code, A.nordre, A.per_codeass, A.per_codecom, B.cli_code, A.sco_code,
            A.trp_code, A.ref_logistique, A.ref_document, C.code_expediteur,A.typ_ordre,A.COMM_INTERNE
        into is_soc_code, is_cam_code, is_nordre, is_per_codeass, is_per_codecom, is_cli_code, is_sco_code,
            is_trp_code, ls_ref_logistique_bw, ls_ref_document_bw, ls_code_expediteur,ls_typ_ordre, ls_comm_interne
    ...
    if is_flu_code = 'COMINV' or is_flu_code = 'PROFOR' or is_flu_code = 'BONLIV' or is_flu_code = 'CASINO' then
        ...
        env_codes.extend();
        env_codes_index := env_codes_index + 1;
        env_codes(env_codes_index) := ls_env_code;
        res := 1;
        msg := 'OK';
        return;
    end if;
...
--        dw_table.object.acces1_1.visible = FALSE
    ...
    if ib_ann_ordre = 'N' then
        ...
        ls_sql := ls_sql || 'from geo_contac K,geo_ordre O ';
        ls_sql := ls_sql ||'where K.flu_code = ''' || is_flu_code || ''' and K.valide = ''O'' and O.ORD_ref = ''' || is_ord_ref || ''' and ';
        ls_sql := ls_sql ||'((K.con_tyt = ''F'' and K.con_tiers in (select geo_ordlig.fou_code from geo_ordlig where geo_ordlig.ord_ref = ''' || is_ord_ref || ''' group by geo_ordlig.fou_code))';
       -- ls_sql := ls_sql ||' and ((O.TYP_ORDRE not in(''RGP'',''ORI'')) OR (O.TYP_ORDRE =''RGP'' and get_flux_rgp (O.ORD_ref , K.con_tiers) = ''SA'') OR (O.TYP_ORDRE =''ORI'' and get_flux_ori (O.ORD_ref , K.con_tiers) = ''BUK'')) ';
		IF ls_typ_ordre ='RGP' THEN
	        ls_sql := ls_sql ||' and get_flux_rgp (O.ORD_ref , K.con_tiers) = ''SA''  ';
		END IF;
		IF ls_typ_ordre ='ORI' THEN
	        ls_sql := ls_sql ||' and get_flux_ori (O.ORD_ref , K.con_tiers) = ''BUK''  ';
		END IF;
    ...
    loop
        ...
        if (is_base_flu_code = 'FICPAO') then
            is_con_fluvar := 'OLD';
        elsif (is_base_flu_code = 'FICPAN') then
            is_con_fluvar := 'NEW';
        end if;
    ...
    update geo_envois set acces2 = (SELECT  FRA_DESC
        ...
        FRA_CODE ='FRET' and
        ROWNUM <= 1)
    where ord_ref = is_ord_ref and trait_exp = 'A' and tyt_code = 'T' and acces2 is null;
    commit;
    ...
    IF is_flu_code ='MINI' then
       of_delete_expedit_uniq(is_ord_ref, res, msg);
    end if;
