...
intNB_PIECE INTEGER;
new_gtin_colis AVI_ART_NORMALISATION.GTIN_COLIS%TYPE;
cursor arts_kit(crt_ord_ref GEO_ORDRE.ord_ref%type, num_ligne GEO_ORDLIG.orl_lig%type, art_kit GEO_ORDLIG.art_ref_kit%type) is
    select orl_ref,gtin_colis_kit,art_ref_kit
    from geo_ordlig
    where ord_ref = crt_ord_ref
    AND art_ref_kit = art_kit
    AND orl_lig > num_ligne;
    ...
    if strEDI_NUM IS NULL OR strEDI_NUM = '' THEN
       if :NEW.ART_REF IS NOT NULL AND (:NEW.ART_REF <> :OLD.ART_REF OR :NEW.ART_REF_KIT IS NULL) THEN
            ...
            SELECT
                ...
                GTIN_COLIS
                INTO strGTIN_COLIS, strGTIN_UC, new_gtin_colis
            ...
            -- Le nouvel article possede un `gtin_colis`
            ELSIF new_gtin_colis is not null then
                :NEW.GTIN_COLIS_KIT := new_gtin_colis;
            END IF;
