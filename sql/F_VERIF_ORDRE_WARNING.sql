...
AS
    ...
    ls_res_bidon varchar2(50);
    ls_msg_bidon varchar2(50);
...
BEGIN
	...
	begin
        select sum(VTE_QTE) into li_totqtevte
        from GEO_ORDLIG
    ...
    if li_totqtevte > 0 and ls_trp_code <> 'CLIENT' and ls_trp_code <> 'STATIO' and ld_tottrp = 0 and ls_inc_rd <> 'D' and ls_facture_avoir = 'F' and (ll_nb_ligne_ordre <> ll_nb_ligne_gratuit) and arg_soc_code <> 'BUK' then
    ...
    -- SL 22/08/2023 : il faut pouvoir facturer si forfait trp et incoterm EXW même si trp_code = 'CLIENT'
   -- IF ls_trp_code = 'CLIENT' THEN
   --   li_ret_forfait := 0;
   -- ELSE
      f_return_forfaits_trp(ls_cen_ref,ls_inc_code,ld_trp_dev_pu_bidon,ls_trp_bta_code_bidon,ls_trp_dev_code_bidon,ls_typ_ordre,ls_res_bidon,ls_msg_bidon,li_ret_forfait);
   -- END IF;

    CASE ls_inc_code
        WHEN 'EXW' THEN
            If ld_trp_pu > 0 and li_ret_forfait = 0 Then
                ls_rc := ls_rc || '(T)  %%% Pour l''incoterm ' || ls_inc_code ||  ' ou le transport CLIENT , le pu tarif transport devrait être nulle' || ls_crlf;
            End If;
        WHEN 'CPT' THEN
        ...
        WHEN 'FCA' THEN
            ...
            End If;
        ELSE
            If ls_trp_code <> 'CLIENT' and ls_trp_code <> 'STATIO'  and (ld_trp_pu is null or ld_trp_pu =0) and ls_facture_avoir = 'F' and arg_soc_code <> 'BUK' Then
                ls_rc := ls_rc || '(T)  %%% Pour l''incoterm ' || ls_inc_code || ', le pu tarif transport devrait être mentionné' || ls_crlf;
            End IF;
            if ls_trp_code <> 'CLIENT' and ls_trp_code <> 'STATIO' and (ls_trp_bta_code is null or ls_trp_bta_code = '') and ls_facture_avoir = 'F' and arg_soc_code <> 'BUK' Then
                ls_rc := ls_rc || '(T)  %%% Pour l''incoterm ' || ls_inc_code || ', l''unité de transport devrait être mentionné' || ls_crlf;
            End If;
    END CASE;

    IF li_ret_forfait > 0 THEN
        IF  ld_trp_dev_pu_bidon <> ld_trp_pu OR ls_trp_bta_code_bidon <> ls_trp_bta_code THEN
            ls_rc    := ls_rc || '(T)  %%% Pour l''incoterm ' || ls_inc_code || ', le cout de transport devait être de ' || to_char(ld_trp_dev_pu_bidon,'0.00') || ' ' || ls_trp_bta_code_bidon || ls_crlf;
        END IF;
    END IF;
    ...
    --OF_VERIF_PALETTE_CHEP (arg_ord_ref, arg_soc_code, ls_cen_code, ll_res, ls_err);
   -- if ll_res <> 1 then
   --     ls_rc := ls_rc ||  '(A) %%% ' || ls_err || ls_crlf;
    --end if;
