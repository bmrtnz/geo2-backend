CREATE OR REPLACE PROCEDURE "OF_INIT_ARTREF_GRP" (
...
begin
	...
	if ls_artref is not null  THEN
		...
		if is_edi_ord is null then
			...
			IF ls_gtin_uc_grp is not null  THEN
			...
			if is_gtin_colis is not null then --Pour ne pas altérer le gtin lors de l'ajout par ref article
