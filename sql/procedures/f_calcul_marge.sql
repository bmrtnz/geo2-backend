...
BEGIN
    ...
    LOOP
        ...
        if  ls_flag_cloture_lig <> 'O' then  -- pour récupérer l'achat qte et vente qté
            ...
            if res <> 1 then
                msg := 'Erreur sur le calcul de la quantité ' || msg;
                return;
            end if;
        end if;
    ...
    EXCEPTION when OTHERS then
        msg := 'Echec de la récuperation des cumuls de quantités et montant des lignes de l''ordre ' || arg_ord_ref;
        res := 0;
        return;
    end;
