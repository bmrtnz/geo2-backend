...
begin
    ...
    into
        ...
        is_inc_code,
        is_inc_lieu
    from GEO_ORDRE O, GEO_DEVISE_REF D, GEO_SOCIETE S
    where O.ORD_REF = arg_ord_ref and
        O.SOC_CODE = S.SOC_CODE and
        S.DEV_CODE = D.DEV_CODE_REF and
        O.DEV_CODE = D.DEV_CODE;
    ...
    begin
        ...
        if  d_ligne = '%%%' then
			delete GEO_ORDRE where ORD_REF =ls_ord_ref_new;
			return;
		end if;
    end;
    commit;
    ...
    f_verif_logistique_ordre(ls_ord_ref_new, res, msg);
    if (msg <> 'OK') then
