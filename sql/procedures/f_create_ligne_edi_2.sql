CREATE OR REPLACE PROCEDURE "F_CREATE_LIGNE_EDI_2" (
...
BEGIN
    -- corresponds à f_create_ligne_edi2.pbl
    res := 0;
    msg := '';
    ...
    if ll_pal_nb_col is not null and ll_pal_nb_col > 0 then
        ll_cde_nb_pal := ROUND( ll_qte_art_cde / ll_pal_nb_col);
        ...
        if ls_arg_bassin = '' or ls_arg_bassin is null  then
            begin
                if ls_trp_code_edi is not null and  ls_trp_code_edi <> '' and ls_bac_code_edi = ls_bac_code then
                    update geo_ordre set trp_code = ls_trp_code_edi, trp_bac_code = ls_bac_code_edi
                    ...
                    if ld_trp_pu_ordre is null or ld_trp_pu_ordre = 0 then
                        update geo_ordre set trp_code = ls_trp_code, trp_dev_pu = ld_trp_pu, trp_dev_code = ls_trp_dev_code, trp_bta_code = ls_trp_bta_code, dev_tx = ld_dev_tx, trp_bac_code = ls_bac_code
                        where ord_ref = arg_ord_ref;
                    else
                        update geo_ordre set trp_code = ls_trp_code, trp_bac_code = ls_bac_code
                        where ord_ref = arg_ord_ref;
                    end if;
                    ls_arg_bassin := ls_bac_code;
                end if;
