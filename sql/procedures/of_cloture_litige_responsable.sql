...
BEGIN
    ...
    begin
        ...
        if res <> 1 then
            msg := 'Erreur sur le calcul de la marge ' || ls_rc;
            return;
        end if;
    end;
    ...
    declare
        flux varchar2(50) := 'RESLIT';
        co SYS_REFCURSOR;
    begin
        If  arg_soc_code <> 'UDC'   then
            GEO_PREPARE_ENVOIS(is_cur_ord_ref, flux, 'O', 'N', arg_username, res, msg, co);
            if res <> 1 then return; end if;
        END IF;
    end;
