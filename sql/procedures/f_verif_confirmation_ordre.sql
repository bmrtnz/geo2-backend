CREATE OR REPLACE PROCEDURE F_VERIF_CONFIRMATION_ORDRE (
...
AS
    ...
    IS
        SELECT  SUM(QUANTITE_COLIS), EAN_PROD_CLIENT, CODE_INTERNE_PROD_CLIENT
        FROM GEO_EDI_LIGNE EL
        WHERE REF_EDI_ORDRE = num_edi
        GROUP BY EAN_PROD_CLIENT, CODE_INTERNE_PROD_CLIENT;
...
BEGIN
    ...
    if ll_num_edi is not null then
        ...
        LOOP
            fetch cur_lig_edi into ll_nb_col_edi, ls_gtin, ls_art_client;
            ...
            WHERE O.REF_EDI_ORDRE = ll_num_edi
              ...
              AND (GTIN_COLIS_KIT = ls_gtin OR GTIN_COLIS_KIT = ls_art_client);
