CREATE OR REPLACE PROCEDURE F_CREE_AVOIR_FOURNI_V2 (
...
AS
    ...
    ls_fou_code GEO_ORDLOG.fou_code%type;
    ls_instructions GEO_ORDLOG.instructions%type;
    ls_fou_ref_doc GEO_ORDLOG.fou_ref_doc%type;
    ls_ref_logistique GEO_ORDLOG.ref_logistique%type;
    ls_ref_document GEO_ORDLOG.ref_document%type;
    ls_valide GEO_ORDLOG.valide%type;
    ls_flag_exped_fournni GEO_ORDLOG.flag_exped_fournni%type;
    ls_datdep_fou_p_yyyymmdd GEO_ORDLOG.datdep_fou_p_yyyymmdd%type;
    ls_plomb GEO_ORDLOG.plomb%type;
    ls_immatriculation GEO_ORDLOG.immatriculation%type;
    ls_DETECTEUR_TEMP GEO_ORDLOG.DETECTEUR_TEMP%type;
    ls_certif_controle GEO_ORDLOG.certif_controle%type;
    ls_certif_phyto GEO_ORDLOG.certif_phyto%type;
    ls_bill_of_lading GEO_ORDLOG.bill_of_lading%type;
    ls_container GEO_ORDLOG.container%type;
    ls_locus_trace GEO_ORDLOG.locus_trace%type;
    ls_comm_interne GEO_ORDRE.comm_interne%type;
...
BEGIN
   ...
   -- on récupère le type de responsable (si fournisseur on remplira la partie achat)
    ...
    begin
        ...
        valide, ttr_code,
        lib_dlv, facture_avoir,typ_ordre,frais_pu, mod_user, comm_interne)
        select
        ...
        valide, ttr_code,
        lib_dlv, 'A','AFO',li_frais_pu,arg_username, substr(ls_comm_interne, 0, 127)
