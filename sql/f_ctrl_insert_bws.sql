...
BEGIN
    ...
    loop
		...
		if ls_bws is not null then
            ...
            if ls_art_ref = arg_art_ref
            and ls_prop_code = arg_prop_code
            and ls_fou_code = arg_fou_code
            and ls_bac_code_station = arg_bac_code_station
            and ll_edi_ordre = arg_ref_edi_ordre
            and ll_edi_ligne = arg_ref_edi_ligne
            then
                res := 1;
				msg := ls_bws || ' déjà ecris';
