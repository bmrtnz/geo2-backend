                ...
                case ls_vte_bta_code_tmp
                    when 'KILO' then
                        ...
                        If (ld_pdnet_colis*ipu.PAL_NB_COL) > 0 Then
                            ld_doua_pu :=ld_doua_pu_tot_new / (ld_pdnet_colis*ipu.PAL_NB_COL);
                            
                            ld_fra_pu := ld_fra_tot  / (ld_pdnet_colis*ipu.PAL_NB_COL);
                        Else
                            ld_doua_pu := 0;
                            ld_fra_pu :=0;
                        End IF;
                    when 'COLIS' then
                        ...
                        If ipu.cde_nb_col > 0 Then
                            ld_doua_pu := ld_doua_pu_tot_new  / ipu.cde_nb_col;
                            
                             ld_fra_pu := ld_fra_tot  / ipu.cde_nb_col;
                        ELse
                            ld_doua_pu := 0;
